...
_____.___.            __               _____________________________
\__  |   |__ __ __ __|  | _______     /  _____/\______   \__    ___/
 /   |   |  |  \  |  \  |/ /\__  \   /   \  ___ |     ___/ |    |   
 \____   |  |  /  |  /    <  / __ \_ \    \_\  \|    |     |    |   
 / ______|____/|____/|__|_ \(____  /  \______  /|____|     |____|   
 \/                       \/     \/          \/                     
...
async def reset(ctx):
    ...
    message_history = {}
    await ctx.send("🤖 Lịch sử tin nhắn bot đã được xóa")
...
@bot.hybrid_command(name="set_chatbot", description="Thiết lập kênh chat cho bot")
async def set_chatbot(ctx, channel: discord.TextChannel):
    if ctx.guild is None:
        await ctx.send("❌ Lệnh này chỉ được sử dụng trong máy chủ")
        return
    ...
    if guild_id in chatbot_channels:
        if chatbot_channels[guild_id]['channel_id'] == str(channel.id):
            ...
            await ctx.send(f"❌ #{channel.name} đã không còn là kênh chat cho bot")
        else:
            chatbot_channels[guild_id] = {'channel_id': str(channel.id)}
            await ctx.send(f"✅ #{channel.name} đã được đặt là kênh chat cho bot. Bot sẽ phản hồi mọi tin nhắn được gửi trong kênh")
    else:
        chatbot_channels[guild_id] = {'channel_id': str(channel.id)}
        await ctx.send(f"✅ #{channel.name} đã được đặt là kênh chat cho bot. Bot sẽ phản hồi mọi tin nhắn được gửi trong kênh")
        ...
        async with message.channel.typing():
            ...
            #Not an Image do text response
                ...
                if "RESET" in cleaned_text:
                    ...
                    await message.channel.send("🤖 Xóa lịch sử cho người dùng: " + str(message.author.name))
                    return
                await message.add_reaction('💬')
...
def get_formatted_message_history(user_id):
    ...
    if user_id in message_history:
        ...
        return "❌ Không có tin nhắn nào được tìm thấy"
